...
local THEME = {
	...
	AccentDark = Color3.fromRGB(255, 20, 147),
	Text = Color3.fromRGB(255, 20, 147),
	DarkText = Color3.fromRGB(255, 20, 147),
	...
	ProgressFill = Color3.fromRGB(255, 20, 147),
}
...
local function createESP(plr)
	...
	hitbox.Color3 = Color3.fromRGB(255, 20, 147)
...
local function updateToggleUI(name, state)
	...
	btn.state = state
	btn.track.BackgroundColor3 = state and Color3.fromRGB(255, 20, 147) or Color3.fromRGB(60, 60, 60)
...
local function local autoPlayEnabled = false
local autoPlayConn = nil
local autoPlayPhase = 1
local autoPlaySide = "left" -- "left" or "right"
local AUTOPLAY_SPEED = 60
-- AutoPlay positions (goes left then right repeatedly)
local APL1  = Vector3.new(-476.48, -6.28, 92.73)
local APLEND = Vector3.new(-483.12, -4.95, 94.80)
local APR1  = Vector3.new(-476.16, -6.52, 25.62)
local APREND = Vector3.new(-483.04, -5.09, 23.14)

local function stopAutoPlay()
	autoPlayEnabled = false
	if autoPlayConn then autoPlayConn:Disconnect() autoPlayConn = nil end
	autoPlayPhase = 1 autoPlaySide = "left"
	if char and hum then hum:Move(Vector3.zero, false) end
	updateToggleUI("Auto Play", false)
	if floatButtons["AutoPlay"] then
		floatButtons["AutoPlay"].state = false
		floatButtons["AutoPlay"].indicator.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
	end
end
...
local function startAutoPlay()
	...
	autoPlayConn = RunService.Heartbeat:Connect(function()
		if not autoPlayEnabled or not char or not hrp or not hum then return end
		local spd = AUTOPLAY_SPEED
		if autoPlaySide == "left" then
			if autoPlayPhase == 1 then
				local d = Vector3.new(APL1.X - hrp.Position.X, 0, APL1.Z - hrp.Position.Z)
				if d.Magnitude < 1 then autoPlayPhase = 2 return end
				local md = d.Unit hum:Move(md, false)
				hrp.AssemblyLinearVelocity = Vector3.new(md.X*spd, hrp.AssemblyLinearVelocity.Y, md.Z*spd)
			elseif autoPlayPhase == 2 then
				local d = Vector3.new(APLEND.X - hrp.Position.X, 0, APLEND.Z - hrp.Position.Z)
				if d.Magnitude < 1 then
					hum:Move(Vector3.zero, false) hrp.AssemblyLinearVelocity = Vector3.new(0,0,0)
					autoPlayPhase = 1 autoPlaySide = "right"
					return
				end
				local md = d.Unit hum:Move(md, false)
				hrp.AssemblyLinearVelocity = Vector3.new(md.X*spd, hrp.AssemblyLinearVelocity.Y, md.Z*spd)
		...
		elseif autoPlaySide == "right" then
			if autoPlayPhase == 1 then
				local d = Vector3.new(APR1.X - hrp.Position.X, 0, APR1.Z - hrp.Position.Z)
				if d.Magnitude < 1 then autoPlayPhase = 2 return end
				local md = d.Unit hum:Move(md, false)
				hrp.AssemblyLinearVelocity = Vector3.new(md.X*spd, hrp.AssemblyLinearVelocity.Y, md.Z*spd)
			elseif autoPlayPhase == 2 then
				local d = Vector3.new(APREND.X - hrp.Position.X, 0, APREND.Z - hrp.Position.Z)
				if d.Magnitude < 1 then
					hum:Move(Vector3.zero, false) hrp.AssemblyLinearVelocity = Vector3.new(0,0,0)
					autoPlayPhase = 1 autoPlaySide = "left"
					return
...
local function setupChar(c)
	...
	if head then
		...
		speedLbl.TextColor3 = Color3.fromRGB(255, 20, 147)
		speedLbl.Font = Enum.Font.Arcade
...
-- ============================================================
-- FPS / PING HUD BAR (titled AURA HUB, top center)
-- ============================================================
local HUDBar = Instance.new("Frame", ScreenGui)
HUDBar.Size = UDim2.new(0, s(340), 0, s(44))
HUDBar.Position = UDim2.new(0.5, -s(170), 0, s(8))
HUDBar.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
HUDBar.BackgroundTransparency = 0.15
HUDBar.BorderSizePixel = 0
Instance.new("UICorner", HUDBar).CornerRadius = UDim.new(0, s(12))
local HUDBarStroke = Instance.new("UIStroke", HUDBar)
HUDBarStroke.Color = Color3.fromRGB(255, 20, 147) HUDBarStroke.Thickness = 1.5
local HUDTitleLbl = Instance.new("TextLabel", HUDBar)
HUDTitleLbl.Size = UDim2.new(0, s(100), 1, 0) HUDTitleLbl.Position = UDim2.new(0, s(10), 0, 0)
HUDTitleLbl.BackgroundTransparency = 1 HUDTitleLbl.Text = "AURA HUB"
HUDTitleLbl.TextColor3 = Color3.fromRGB(255, 20, 147) HUDTitleLbl.Font = Enum.Font.Arcade HUDTitleLbl.TextSize = s(13)
HUDTitleLbl.TextXAlignment = Enum.TextXAlignment.Left
local HUDDiv1 = Instance.new("Frame", HUDBar) HUDDiv1.Size = UDim2.new(0, 1, 0.6, 0) HUDDiv1.Position = UDim2.new(0, s(118), 0.2, 0)
HUDDiv1.BackgroundColor3 = Color3.fromRGB(255, 20, 147) HUDDiv1.BackgroundTransparency = 0.5 HUDDiv1.BorderSizePixel = 0
FPSLabel = Instance.new("TextLabel", HUDBar) FPSLabel.Size = UDim2.new(0, s(90), 1, 0) FPSLabel.Position = UDim2.new(0, s(128), 0, 0)
FPSLabel.BackgroundTransparency = 1 FPSLabel.Text = "FPS: --" FPSLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
FPSLabel.Font = Enum.Font.Arcade FPSLabel.TextSize = s(12) FPSLabel.TextXAlignment = Enum.TextXAlignment.Left
local HUDDiv2 = Instance.new("Frame", HUDBar) HUDDiv2.Size = UDim2.new(0, 1, 0.6, 0) HUDDiv2.Position = UDim2.new(0, s(225), 0.2, 0)
HUDDiv2.BackgroundColor3 = Color3.fromRGB(255, 20, 147) HUDDiv2.BackgroundTransparency = 0.5 HUDDiv2.BorderSizePixel = 0
PingLabel = Instance.new("TextLabel", HUDBar) PingLabel.Size = UDim2.new(0, s(100), 1, 0) PingLabel.Position = UDim2.new(0, s(234), 0, 0)
PingLabel.BackgroundTransparency = 1 PingLabel.Text = "Ping: --ms" PingLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
PingLabel.Font = Enum.Font.Arcade PingLabel.TextSize = s(12) PingLabel.TextXAlignment = Enum.TextXAlignment.Left
local _hudFC, _hudLT = 0, tick()
RunService.RenderStepped:Connect(function()
	_hudFC = _hudFC + 1
	local _ct = tick()
	if _ct - _hudLT >= 1 then
		if FPSLabel then FPSLabel.Text = string.format("FPS: %d", _hudFC) end
		_hudFC = 0 _hudLT = _ct
	end
...
ProgressPercentLabel.Font = Enum.Font.Arcade
...
RadiusLabelText.Font = Enum.Font.Arcade
...
RadiusInput.Font = Enum.Font.Arcade
...
Title.Text = "AURA HUB" Title.TextColor3 = THEME.Accent Title.Font = Enum.Font.Arcade
...
SubTitle.Text = "Made // By // lotrix" SubTitle.TextColor3 = THEME.Accent SubTitle.Font = Enum.Font.Arcade
...
CredsLabel.Text = "" CredsLabel.TextColor3 = THEME.DarkText CredsLabel.Font = Enum.Font.Arcade
...
local function createSection(name, order)
	...
	lbl.Font = Enum.Font.Arcade lbl.TextSize = s(11) lbl.TextXAlignment = Enum.TextXAlignment.Left
end
...
local function createToggle(name, keyHintPC, keyHintController, defaultState, callback, order)
	...
	if badgeText and badgeText ~= "?" and badgeText ~= "" then
		...
		KL.TextColor3 = Color3.fromRGB(0, 0, 0) KL.Font = Enum.Font.Arcade KL.TextSize = s(12)
	end
	...
	Label.Font = Enum.Font.Arcade Label.TextSize = s(14) Label.TextXAlignment = Enum.TextXAlignment.Left
	...
	Track.BackgroundColor3 = defaultState and Color3.fromRGB(255, 20, 147) or Color3.fromRGB(60, 60, 60) Track.BorderSizePixel = 0
	...
	Btn.MouseButton1Click:Connect(function()
		...
		Track.BackgroundColor3 = ns and Color3.fromRGB(255, 20, 147) or Color3.fromRGB(60, 60, 60)
...
local function createTextInput(name, defaultVal, minVal, maxVal, callback, order, decimals)
	...
	Label.BackgroundTransparency = 1 Label.Text = name Label.TextColor3 = THEME.Text
	Label.Font = Enum.Font.Arcade Label.TextSize = s(13) Label.TextXAlignment = Enum.TextXAlignment.Left
	...
	ValueBox.Font = Enum.Font.Arcade ValueBox.TextSize = s(14) ValueBox.TextXAlignment = Enum.TextXAlignment.Right
...
local function createSlider(name, defaultVal, minVal, maxVal, callback, order, decimals)
	local SF = Instance.new("Frame", Content)
	SF.Size = UDim2.new(1, 0, 0, s(64)) SF.BackgroundColor3 = THEME.Card SF.BorderSizePixel = 0 SF.LayoutOrder = order
	Instance.new("UICorner", SF).CornerRadius = UDim.new(0, s(10))
	local fmtV = decimals and function(v) return string.format("%.1f", v) end or function(v) return tostring(math.floor(v)) end
	local currentVal = defaultVal

	local Label = Instance.new("TextLabel", SF)
	Label.Size = UDim2.new(1, -s(80), 0, s(22)) Label.Position = UDim2.new(0, s(14), 0, s(6))
	Label.BackgroundTransparency = 1 Label.Text = name Label.TextColor3 = THEME.Text
	Label.Font = Enum.Font.Arcade Label.TextSize = s(13) Label.TextXAlignment = Enum.TextXAlignment.Left

	local ValLbl = Instance.new("TextLabel", SF)
	ValLbl.Size = UDim2.new(0, s(60), 0, s(22)) ValLbl.Position = UDim2.new(1, -s(68), 0, s(6))
	ValLbl.BackgroundTransparency = 1 ValLbl.Text = fmtV(defaultVal) ValLbl.TextColor3 = THEME.Accent
	ValLbl.Font = Enum.Font.Arcade ValLbl.TextSize = s(14) ValLbl.TextXAlignment = Enum.TextXAlignment.Right

	local TrackBg = Instance.new("Frame", SF)
	TrackBg.Size = UDim2.new(1, -s(28), 0, s(14)) TrackBg.Position = UDim2.new(0, s(14), 0, s(36))
	...
	Knob.Size = UDim2.new(0, s(18), 0, s(18)) Knob.AnchorPoint = Vector2.new(0.5, 0.5)
	Knob.Position = UDim2.new(initPct, 0, 0.5, 0)
	Knob.BackgroundColor3 = Color3.fromRGB(255,255,255) Knob.BorderSizePixel = 0
	Instance.new("UICorner", Knob).CornerRadius = UDim.new(1, 0)
	local KnobStroke = Instance.new("UIStroke", Knob) KnobStroke.Color = THEME.Accent KnobStroke.Thickness = 2
	...
	local HitBox = Instance.new("TextButton", SF)
	HitBox.Size = UDim2.new(1, -s(28), 0, s(30)) HitBox.Position = UDim2.new(0, s(14), 0, s(29))
	HitBox.BackgroundTransparency = 1 HitBox.Text = "" HitBox.ZIndex = 5

	local function getPct(input)
		local absPos = TrackBg.AbsolutePosition.X
		local absSize = TrackBg.AbsoluteSize.X
		return math.clamp((input.Position.X - absPos) / absSize, 0, 1)
	end
...
local function createCarrySpeedInput(order)
	...
	Label.TextColor3 = THEME.Text Label.Font = Enum.Font.Arcade
	...
	CarrySpeedInput.Font = Enum.Font.Arcade
...
local function createKeybindSetter(actionName, defaultPC, defaultController, order)
	...
	KeyBtn.TextColor3 = THEME.Text KeyBtn.Font = Enum.Font.Arcade KeyBtn.TextSize = s(11) KeyBtn.BorderSizePixel = 0
...
local function createCombatKeybindBadge(parent, actionName)
	...
	badge.BackgroundColor3 = Color3.fromRGB(255, 20, 147)
	...
	badge.MouseButton1Click:Connect(function()
		...
		conn = UserInputService.InputBegan:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.Keyboard then
				KEYBINDS[actionName].PC = input.KeyCode
				badge.BackgroundColor3 = Color3.fromRGB(255, 20, 147) listeningForKeybind = nil
				saveConfig() syncKeybindDisplays(actionName) conn:Disconnect()
			end
		end)
...
createToggle("Aura Mode", "?", "?", galaxyEnabled, function(v)
	galaxyEnabled = v nightSkyEnabled = v
	if v then startGalaxy() enableVoidMode() else stopGalaxy() disableVoidMode() end
end, 11)
createSlider("Hop Power", GALAXY_HOP_POWER, 5, 100, function(v) GALAXY_HOP_POWER = v end, 12, true)
createSlider("Gravity %", GALAXY_GRAVITY_PERCENT, 10, 100, function(v)
...
createToggle("Auto Play", "?", "?", false, function(v)
	if v then autoPlayEnabled = true startAutoPlay() else stopAutoPlay() end
end, 36)
...
SaveBtn.TextColor3 = Color3.fromRGB(255, 255, 255) SaveBtn.Font = Enum.Font.Arcade SaveBtn.TextSize = s(16)
...
ToggleBtn.BackgroundColor3 = THEME.FloatButton ToggleBtn.Text = "A" ToggleBtn.TextColor3 = THEME.Accent
ToggleBtn.Font = Enum.Font.Arcade ToggleBtn.TextSize = s(32) ToggleBtn.Active = true ToggleBtn.BorderSizePixel = 0
...
local function createFloatButton(name, topLabel, bottomLabel, order, callback)
	...
	TopLbl.TextColor3 = THEME.Accent TopLbl.Font = Enum.Font.Arcade TopLbl.TextSize = s(15)
	...
	BotLbl.TextColor3 = THEME.DarkText BotLbl.Font = Enum.Font.Arcade BotLbl.TextSize = s(13)
	local dotSz = s(7)
...
createFloatButton("FloatBtn", "FLOAT", "MODE", 6, function(state)
	...
	floatButtons["FloatBtn"].state = floatEnabled
	floatButtons["FloatBtn"].indicator.BackgroundColor3 = floatEnabled and THEME.Accent or THEME.ToggleOff
	updateToggleUI("Float", floatEnabled)
end)
...
createFloatButton("AutoPlay", "AUTO", "PLAY", 5, function(state)
	if state then
		...
		floatButtons["AutoLeft"].state = false floatButtons["AutoLeft"].indicator.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
		floatButtons["AutoRight"].state = false floatButtons["AutoRight"].indicator.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
		startAutoPlay()
	else
		stopAutoPlay()
	end
	floatButtons["AutoPlay"].state = autoPlayEnabled
	floatButtons["AutoPlay"].indicator.BackgroundColor3 = autoPlayEnabled and THEME.Accent or THEME.ToggleOff
	updateToggleUI("Auto Play", autoPlayEnabled)
end)
...
task.spawn(function()
	task.wait(0.3)
	updateToggleUI("Aura Mode", galaxyEnabled)
